...
import com.ecomarket.cl.ecomarket.repository.BoletaRepository;
...
public class CarritoCompraService {
    ...
    public Optional<CarritoCompra> obtenerPorId(String rut) {
        return carritoCompraRepository.findByCliente_Rut(rut);
    }
    ...
    public CarritoCompra aplicarCuponAlCarrito(String clienteRut, String codigoCupon) {
        Optional<CarritoCompra> carritoOpt = carritoCompraRepository.findByCliente_Rut(clienteRut);
        ...
        carrito.calcularTotal(); // actualizar total antes de aplicar descuento
    ...
    public Boleta realizarCompra(String clienteRut) {
    Optional<CarritoCompra> carritoOpt = carritoCompraRepository.findByCliente_Rut(clienteRut);
    if (carritoOpt.isEmpty()) {
        throw new RuntimeException("Carrito no encontrado para el cliente con RUT: " + clienteRut);
    }
    ...
    if (carrito.getProductos() == null || carrito.getProductos().isEmpty()) {
        throw new RuntimeException("El carrito está vacío. No se puede generar una boleta.");
    }

    Boleta boleta = new Boleta();
    boleta.setClienteRut(clienteRut);
    boleta.setProductos(new ArrayList<>(carrito.getProductos()));  
    boleta.setTotal(carrito.getTotalCarro());
    boleta.setCuponAplicado(carrito.isCuponAplicado());
    boleta.setDescuentoAplicado(carrito.getDescuentoAplicado());
    ...
    carrito.setTotalCarro(0.0);
    carrito.setCuponAplicado(false);
